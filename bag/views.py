        ...
        if size:
            ...
            if item_id in list(bag.keys()):
                if weight in bag[item_id]["items_by_weight"].keys():
                    ...
                    messages.success(
                        request,
                        f'Updated weight {weight.capitalize()} {product.name} \
                            quantity to \
                    ...
                    messages.success(request, f"Added \
                        {weight.capitalize()} {product.name} to your bag")
            else:
                bag[item_id] = {"items_by_weight": {weight: quantity}}
                messages.success(request, f"Added \
                    {weight.capitalize()} {product.name} to your bag")
    ...
    if size or weight:
        if size:
            ...
            if quantity > 0:
                ...
                messages.success(
                    request,
                    f'Updated {weight.capitalize()} {product.name} quantity \
                        to {bag[item_id]["items_by_weight"][weight]}',
                )
            else:
                del bag[item_id]["items_by_weight"][weight]
                if not bag[item_id]["items_by_weight"]:
                    bag.pop(item_id)
                messages.success(request, f"Removed {weight.capitalize()} \
                    {product.name} from your bag")
    else:
        if quantity > 0:
            bag[item_id] = quantity
